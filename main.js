...
let mixer, clock,  actions, activeAction, previousAction, deskModel, chairModel, ballModel, humanModel,  desktopMixer, desktopAction, bowMixer, ballmixer, bowAction, ballAction, currentAnimation, discoModel
...
renderer.setPixelRatio(sizes.pixels);
...
function createDiscoLight (color) {
...
discoloader.load(disco.href, (gltf) =>{
...
let lastAnimation 
function checkModels(human, fadeToAction ,name) {
...
function animationToggle (animations, human) {
  actions = {}
  if(animations, human) {
  ...
  clipCntrl?.onChange(() =>{
    ...
    scene.remove(chairModel, deskModel)
...
function toggleAnimation (lastAnimation , firstAnimation) {
  scene.remove(planeMesh)

  if(firstAnimation === 'dance') {
    controls.minDistance = 5
    controls.maxDistance = 25;
    gsap.to(camera.position, {z: 4.5 ,x : -13, y: 2  , duration : 2, ease: 'power4.inOut'})
